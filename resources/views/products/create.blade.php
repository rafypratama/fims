    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500">Tambahkan produk baru ke dalam katalog customer tertentu.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden p-6">
            <form method="POST" action="{{ route('products.store') }}" class="space-y-5">
                ...
                <div class="space-y-1">
                    ...
                    <select id="customer_id" name="customer_id" required class="w-full py-2 px-3 text-sm border border-slate-250 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors @error('customer_id') border-rose-500 @enderror">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Contoh: Dus Kemasan Roti" required class="w-full py-2 px-3 text-sm border border-slate-250 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors @error('name') border-rose-500 @enderror" />
                    @error('name')
                        <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" id="code" name="code" value="{{ old('code') }}" placeholder="Contoh: DUS-001" class="w-full py-2 px-3 text-sm border border-slate-250 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors" />
                    </div>

                    <!-- Unit -->
                    <div class="space-y-1">
                        <label for="unit" class="text-sm font-semibold text-slate-700">Satuan <span class="text-rose-500">*</span></label>
                        <input type="text" id="unit" name="unit" value="{{ old('unit', 'Pcs') }}" placeholder="Contoh: Pcs, Box, Rim, kg" required class="w-full py-2 px-3 text-sm border border-slate-250 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors @error('unit') border-rose-500 @enderror" />
                        @error('unit')
                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="default_price" name="default_price" value="{{ old('default_price', 0) }}" min="0" step="1" required class="w-full py-2 pl-9 pr-3 text-sm border border-slate-250 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 transition-colors @error('default_price') border-rose-500 @enderror" />
                    </div>
                    @error('default_price')
                        <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                    @enderror
